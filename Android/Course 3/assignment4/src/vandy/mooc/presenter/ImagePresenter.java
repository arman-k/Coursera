...
public class ImagePresenter 
    ...
    public void startProcessing() {
        if (mUrlList.isEmpty())
            ...
            //Pick an url from the list of URLs and process them
            for (Uri url: mUrlList)
            {
            	FilterAsyncTask mFilterAsyncTask = new FilterAsyncTask(getApplicationContext(), ImagePresenter.this); //TODO Alternative - won't be needing ImagePresenter.this
            	DownloadAsyncTask mDownloadAsyncTask = new DownloadAsyncTask(getApplicationContext(), ImagePresenter.this, mFilterAsyncTask);
            	mDownloadAsyncTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, url, mDirectoryPathname);
            	//TODO Alternative - Do the onProcessingComplete here instead of in onPostExecute in FilterAsyncTask if the above doesn't work. Also fix the other files
				//Please modify the ImageLoader class. result.getmImageViewHolder() should be repositioned
				//Delete original files, only keep filtered files
				//Implement another filter if you have time
				//Use weak reference for contexts as described here: https://class.coursera.org/posaconcurrency-003/forum/thread?thread_id=224#comment-410
            }
        }
    }
